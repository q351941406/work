    ...
    while (instanceClass != [NSObject class]) {
        ...
        for (int i = 0; i < propertyListCount; i++) {
            ...
            if (propertyName && !readonly) {
                ...
                BOOL flag = [NSObject gzw_isNSObjectClass:propertyValueClass];
                if (flag) {
                    if ([propertyValue conformsToProtocol:@protocol(NSCopying)]) {
                        ...
                        [copyValue gzw_easyDeepCopy:propertyValue];
                        [self setValue:copyValue forKey:propertyName];
                    }
                }else{
                    [self setValue:propertyValue forKey:propertyName];
...
+ (BOOL)gzw_isNSObjectClass:(Class)clazz{
    ...
    if (flag) {
        ...
        if (!superClass) {
            ...
            return  [NSObject gzw_isNSObjectClass:superClass];
        }
    }
}
...
- (void)gzw_performOnMainThread:(void(^)(void))block wait:(BOOL)shouldWait {
    if (shouldWait) {
        // Synchronous
...
{
    return [[self class] gzw_propertyKeys];
}
+ (NSArray *)gzw_propertyKeys {
...
{
    return [[self class] gzw_propertiesInfo];
}
/**
...
+ (NSArray *)gzw_propertiesInfo
{
    ...
    for (int i = 0; i < propertyCount; i++)
    {
        [propertieArray addObject:({
            ...
            NSDictionary *dictionary = [self gzw_dictionaryWithProperty:properties[i]];
...
+ (NSArray *)gzw_propertiesWithCodeFormat
{
    ...
    NSArray *properties = [[self class] gzw_propertiesInfo];
...
-(NSArray*)gzw_methodListInfo{
    ...
    for (int i = 0; i < count ; i++)
    {
        ...
        for (int index=0; index<argumentsCount; index++) {
            ...
            [arguments addObject:[[self class] gzw_decodeType:arg]];
        }
        
        NSString *returnTypeString =[[self class] gzw_decodeType:returnType];
        NSString *encodeString = [[self class] gzw_decodeType:encoding];
...
-(NSDictionary *)gzw_protocolList{
    return [[self class]gzw_protocolList];
}
+ (NSDictionary *)gzw_protocolList
{
...
+ (NSArray *)gzw_instanceVariable
{
    ...
    NSMutableArray *result = [NSMutableArray array];
    for (int i = 0; i < outCount; i++) {
        NSString *type = [[self class] gzw_decodeType:ivar_getTypeEncoding(ivars[i])];
...
#pragma mark -- help
+ (NSDictionary *)gzw_dictionaryWithProperty:(objc_property_t)property
{
...
+ (NSString *)gzw_decodeType:(const char *)cString
{
    ...
    } else
    {
        if ([[result substringToIndex:1] isEqualToString:@"^"]) {
            result = [NSString stringWithFormat:@"%@ *",
                      [NSString gzw_decodeType:[[result substringFromIndex:1] cStringUsingEncoding:NSUTF8StringEncoding]]];
        }
    }
    return result;
}
BOOL gzw_method_swizzle(Class klass, SEL origSel, SEL altSel)
{
    if (!klass)
        return NO;
...
void gzw_method_append(Class toClass, Class fromClass, SEL selector)
{
...
void gzw_method_replace(Class toClass, Class fromClass, SEL selector)
{
    if (!toClass || !fromClass || ! selector)
        return;
...
{
    gzw_method_swizzle(self.class, originalMethod, newMethod);
}

+ (void)gzw_appendMethod:(SEL)newMethod fromClass:(Class)klass
{
    gzw_method_append(self.class, klass, newMethod);
}

+ (void)gzw_replaceMethod:(SEL)method fromClass:(Class)klass
{
    gzw_method_replace(self.class, klass, method);
}

- (BOOL)gzw_respondsToSelector:(SEL)selector untilClass:(Class)stopClass
{
    return [self.class gzw_instancesRespondToSelector:selector untilClass:stopClass];
}
...
{
    return [self.superclass gzw_instancesRespondToSelector:selector untilClass:stopClass];
}
