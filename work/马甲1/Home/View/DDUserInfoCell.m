...
-(NSString *)getPersonInfoStr
{
    ...
    if (_detailModel.birthDate.length>=10) {
        ...
        string = [NSString stringWithFormat:@"%@年%@%@孩,在本App发布过%@条任务,完成过%@条任务.",[_detailModel.birthDate substringWithRange:NSMakeRange(2, 2)],[DateOrTimeTool getConstellation:_detailModel.birthDate],_detailModel.sex.integerValue==2?@"男":@"女",_detailModel.publishDemandCount,_detailModel.completedDemandCount];
    }else{
        string = [NSString stringWithFormat:@"该%@孩,在本App发布过%@条任务,完成过%@条任务.",_detailModel.sex.integerValue==2?@"男":@"女",_detailModel.publishDemandCount,_detailModel.completedDemandCount];
    }
    return string;
}
...
-(NSString *)getSkillPersonInfoStr
{
    ...
    if (_skillDetailM.birthDate.length>=10) {
        ...
        string = [NSString stringWithFormat:@"%@年%@%@孩,兼职技能达人,在本App发布过%@条任务,完成过%@条任务.",[_skillDetailM.birthDate substringWithRange:NSMakeRange(2, 2)],[DateOrTimeTool getConstellation:_skillDetailM.birthDate],_skillDetailM.sex==2?@"男":@"女",[NSString stringWithFormat:@"%ld",_skillDetailM.publishDemandCount],[NSString stringWithFormat:@"%ld",_skillDetailM.completedDemandCount]];
    }else{
        string = [NSString stringWithFormat:@"该%@孩,兼职技能达人,在本App发布过%@条任务,完成过%@条任务.",_skillDetailM.sex==2?@"男":@"女",[NSString stringWithFormat:@"%ld",_skillDetailM.publishDemandCount],[NSString stringWithFormat:@"%ld",_skillDetailM.completedDemandCount]];
    }
