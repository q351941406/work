...
-(void)requestMineInfo
{
    ...
    [JGHTTPClient getMineInfoSuccess:^(id responseObject) {
        ...
        NSMutableAttributedString *descriptionStr = [[NSMutableAttributedString alloc] initWithString:[NSString stringWithFormat:@"我已经在本App里赚取了%.2f元啦",ownMoney]];
        NSString *aaa = [NSString stringWithFormat:@"%.2f",ownMoney];
        NSRange range = [[NSString stringWithFormat:@"我已经在本App里赚取了%.2f元啦",ownMoney] rangeOfString:aaa];
...
- (IBAction)skillConfirm:(id)sender {
    ...
    UIAlertView *a = [[UIAlertView alloc]initWithTitle:@"需实名认证后方可达人认证" message:nil delegate:nil cancelButtonTitle:@"好的" otherButtonTitles:nil];
    [a show];
